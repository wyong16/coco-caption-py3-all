...
class HybridLoader:
    ...
    def __init__(self, db_path, ext, img_path=None):
    ...
    def img_loader(self, imgpath):
        image = Image.open(os.path.join(imgpath)).convert('RGB')
    ...
    def get(self, key, key_img=None):
        ...
        # load image
        if (self.img_path and key_img) is not None:
            img = self.img_loader(os.path.join(self.img_path, key_img + self.ext))
...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt, transform=None):
        ...
        self.att_loader = HybridLoader(self.opt.input_att_dir, '.jpg',img_path="/kaggle/input/mscoco2014-resized")
    ...
    def get_batch(self, split, batch_size=None):
        ...
        #     data['att_masks'] = None
        data['att_feats'] = torch.stack(att_batch)
    ...
    def __getitem__(self, index):
        ...
        self.att_loader.transform = self.transform_val if self.info['images'][ix]['split'] in ("val","test") else self.transform_train
        file_name = "train2014_resized" if self.info['images'][ix]['split'] == "train"  else "val2014_resized"
        if self.use_att:
            # att_feat = self.att_loader.get(str(self.info['images'][ix]['id']))
            att_feat = self.att_loader.get(str(self.info['images'][ix]['file_path']), key_img = os.path.join(file_name,str(self.info['images'][ix]['file_path'].split(r".")[0])))
            # Reshape to K x C
