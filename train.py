...
import traceback
import sys
sys.path.append("/kaggle/input/captioncodepy3")
...
from resnet_util import ResnetBackbone
#from FGM import FGM
...
def repeat_feat(att_feats, ncap_per_img=5):
    if isinstance(att_feats, list):
        ret_feats = []
        for feats in att_feats:
            b, n, h = feats.size()
            batchsize_cap = b * ncap_per_img
            feats = feats.unsqueeze(1).expand(b, ncap_per_img, n, h)
            feats = feats.reshape(batchsize_cap, n, h)
            ret_feats.append(feats)
    else:
        b, n, h = att_feats.size()
        batchsize_cap = b * ncap_per_img
        feats = att_feats.unsqueeze(1).expand(b, ncap_per_img, n, h)
        ret_feats = feats.reshape(batchsize_cap, n, h)
    return ret_feats
