...
class NoamOpt(object):
    ...
    def step(self, epoch):
        ...
        for p in self.optimizer.param_groups:
            p['lr'] = rate
        if self.cur_epoch != self._step:
            self._rate = rate
            self.cur_epoch = self._step
    ...
    def rate(self, step = None):
        ...
        if step is None:
            step = self._step - self.begin_epoch
        # return self.factor * \
        #     (self.model_size ** (-0.5) *
        #     min(step ** (-0.5), step * self.warmup ** (-1.5)))
        if step < 6:
            # return min(step * 10**(-4),5 * 10**(-4))
            return min(step * 3 * 10**(-6),1 * 10**(-5))
        elif step % 3 == 0 :
            return self._rate / 2
        else:
            return self._rate
